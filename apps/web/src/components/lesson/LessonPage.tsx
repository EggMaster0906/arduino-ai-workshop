...
export function LessonPage({ course }: { course: Course }) {
  ...
  return <div className="lesson-page" data-testid="lesson-page">
    ...
    {promptBuilder && <section className="next-step-card"><div><p className="eyebrow">下一個任務</p><h2>把你的想法整理成 Prompt</h2><p>先說清楚需求，再把完成的 Prompt 交給你使用的外部生成式 AI。</p></div><Link className="primary-button" to={`/prompt?task=${promptBuilder.taskId ?? ''}&returnLevel=${level.id}`}>前往 Prompt Builder</Link></section>}
